...
let blackjackAdminSettings = {
    luckFactor: 0,
    isManualShuffle: false,
};
let adminLotteryChoice = null; // Global for manual lottery result
...
app.post('/api/purchase-plan', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // **All investments require admin approval initially**
        const investmentStatus = 'pending_admin_approval'; 
        
        // **Deduct the price from the user's total balance**
        ...
        if (investmentError) {
            ...
            // FIX: Revert the balance deduction if investment recording fails (CRITICAL)
            await supabase.rpc('increment_user_balance_or_withdrawable', { p_user_id: req.user.id, p_amount: price }); 
            return res.status(500).json({ error: 'Purchase failed after payment. Please contact support immediately.' });
        }
        
        // **Send a detailed admin notification for ALL purchases**
        await supabase.from('notifications').insert({
            user_id: 1, // Assuming admin user ID is 1
            type: 'admin_action_required',
            message: `Investment Approval Needed: User ID ${req.user.id} purchased plan "${name}" for ${formatCurrency(price)}. Investment ID: ${investment.id}`
        });
...
app.post('/api/aviator/cashout', authenticateToken, async (req, res) => {
    ...
    // This is placeholder logic, actual cashout needs to check for a bet in DB and process payout
    const betAmount = 0; // Replace with actual bet amount from DB
...
// Removed duplicate Pushpa game state declarations
...
// Removed duplicate aviator game state declarations
...
const processLotteryRound = async (roundId) => {
    ...
    // Clear manual choice after it's used
    if (adminLotteryChoice && adminLotteryChoice.roundId === roundId) {
        adminLotteryChoice = null;
    }
...
async function processRoundResults(period) {
    ...
    try {
        ...
        if (gameState.mode === 'admin' && gameState.next_result !== null) {
            ...
            // Auto mode logic
            const totalBetIn = bets.reduce((sum, b) => sum + (b.amount || 0), 0);
            const netResults = totalPayouts.map(payout => totalBetIn - payout);
            
            let targetNetResult = -Infinity;
            if (gameState.payout_priority === 'admin') {
                targetNetResult = Math.max(...netResults);
            } else if (gameState.payout_priority === 'users') {
                targetNetResult = Math.min(...netResults);
            }

            const targetIndices = netResults.map((net, i) => net === targetNetResult ? i : -1).filter(i => i !== -1);
            winningNumber = targetIndices.length > 0 ? targetIndices[Math.floor(Math.random() * targetIndices.length)] : Math.floor(Math.random() * 10);
        ...
        await supabase.from('game_state').update({ next_result: null }).eq('id', 1); // Clear admin result
...
async function gameLoop() {
    ...
    try {
        ...
        // Process results of the previous round
        await processRoundResults(previousPeriod); 
...
app.get('/api/admin/pushpa-analysis', authenticateAdmin, async (req, res) => {
    try {
        ...
        // FIX: The original code returned a random number here on error/no bets, which is wrong for analysis. Correcting.
        if (error) throw error; 
...
app.get('/api/admin/investments/pending', authenticateAdmin, async (req, res) => {
    try {
        ...
        const { data, error } = await supabase
            .from('investments')
            .select('id, user_id, amount, plan_name, created_at')
            .eq('status', 'pending_admin_approval')
            .order('created_at', { ascending: true });

        if (error) throw error;

        // The frontend expects the data to be under 'pendingRequests'
...
// --- **NEW ENDPOINTS FOR INVESTMENT APPROVAL ACTIONS** ---
app.post('/api/admin/investments/approve', authenticateAdmin, async (req, res) => {
    const { id } = req.body;
    try {
        const { data: investment, error: fetchError } = await supabase.from('investments').select('*').eq('id', id).single();
...
await supabase.from('investments').update({ status: 'active', activated_at: new Date().toISOString() }).eq('id', id);
        ...
        await supabase.from('notifications').insert({
            ...
            type: 'investment',
            message: `Your investment in the "${investment.plan_name}" plan has been approved and is now active!`
        });
        
        // 3. Optional: Trigger immediate distribution if logic allows, or just rely on cron.
        // For simplicity, we just rely on the cron/manual admin distribution.
        
        res.json({ message: `Investment #${id} approved and is now active.` });
    } catch (err) {
...
app.post('/api/admin/withdrawal/:id/reject', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        // FIX: The rejected amount needs to be credited back to the user's withdrawable wallet.
        await supabase.rpc('increment_user_withdrawable_wallet', { p_user_id: withdrawal.user_id, p_amount: withdrawal.amount }); 
        await supabase.from('notifications').insert({
            user_id: withdrawal.user_id,
            type: 'withdrawal',
            message: `Your withdrawal of ₹${withdrawal.amount.toLocaleString()} was rejected and the funds have been returned to your withdrawable wallet.`
        });

        res.json({ message: 'Withdrawal rejected successfully and funds returned to user.' });
    } catch (err) {
...
app.post('/api/admin/lottery-mode', authenticateAdmin, async (req, res) => {
    const { mode } = req.body;
    if (['auto', 'admin'].includes(mode)) {
        // FIX: Update game_state table to persist the mode setting
        await supabase.from('game_state').update({ lottery_mode: mode }).eq('id', 1);
...
async function dailyInvestmentUpdate() {
    ...
    try {
        // FIX: Ensuring only active investments are processed for day decrement
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    // FIX: Start game loops and the daily interval here, only once.
    gameLoop(); 
    runPushpaGameCycle();
    runAviatorCycle(); 
